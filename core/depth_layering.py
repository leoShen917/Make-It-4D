...
def get_depth_bins(depth=None, disparity=None, num_bins=None):
    ...
    # near_disp = disparity * (1 - sky[None].float().cuda())
    # disparity_max = near_disp.max().item()
    # disparity_min = torch.unique(near_disp)[2].item()
    # disparity_feat = near_disp[:, :, ::10, ::10].reshape(-1, 1).cpu().numpy()
    # disparity_feat = disparity_feat[disparity_feat!=0].reshape(-1, 1)
    ...
    depth_bins[0] = depth.min() - 1e-4
    # depth_bins[-1] = 1/disparity_min
    depth_bins[-1] = depth.max() + 1e-4 if depth.max() + 1e-4 > depth_bins[-1] else depth_bins[-1] + 1e-4
    return depth_bins
